    ...
    return (
        <div className="space-y-8 p-4 text-foreground">
            <div className="flex justify-between items-center border-b pb-6">
                <div>
                    <h1 className="text-4xl font-bold flex items-center gap-2 font-cairo">
                        <Users className="w-8 h-8 text-primary" /> {workspace.name}
                    </h1>
                    <p className="text-muted-foreground mt-2 font-tajawal">{workspace.description}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-6">
                    <Card>
                        <CardHeader className="flex flex-row justify-between items-center">
                            <div>
                                <CardTitle className="font-cairo">جلسات الفريق</CardTitle>
                                <CardDescription className="font-tajawal">أحدث العصف الذهني</CardDescription>
                            </div>
                            <Button size="sm"><Plus className="w-4 h-4 mr-1" /> جلسة جديدة</Button>
                        </CardHeader>
                        <CardContent>
                            {workspace.sessions.map(s => (
                                <div key={s.id} className="flex justify-between items-center py-3 border-b last:border-0 font-tajawal">
                ...
                <div className="md:col-span-1 space-y-6">
                    <Card>
                        <CardHeader>
                            <CardTitle className="font-cairo">الأعضاء ({workspace.members.length})</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {workspace.members.map(m => (
                                <div key={m.id} className="flex justify-between items-center font-tajawal">
