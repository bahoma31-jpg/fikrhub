...
export default async function AnalyticsPage() {
    ...
    return (
        <div className="space-y-8 p-4 text-foreground">
            <h1 className="text-4xl font-bold">الإحصائيات</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <ProgressTracker data={{ completed: 5, total: 20 }} />
            </div>

            {/* Top Ideas */}
            <div className="space-y-4">
                <h2 className="text-2xl font-bold mt-8">أفضل الأفكار المقيّمة</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {analytics.topIdeas.map((idea, i) => (
                        <Card key={i} className="hover:shadow-md transition-shadow">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-lg">فكرة رائعة</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <p className="font-medium text-muted-foreground">{idea.text}</p>
                                <div className="mt-4 flex justify-between items-center text-sm font-bold text-primary">
                                    <span>التقييم:</span>
